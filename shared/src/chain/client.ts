...
import { buildProofSnapshot } from "../proof/buildProofSnapshot.js";
import { registryAbi, rewardDistributorAbi, verifierAbi } from "./abis.js";
import type { ChainConfig, JobStateName, ProofSnapshot, VerificationRecord } from "../types.js";
...
export function createChainClient(config: ChainConfig) {
  ...
  return {
    ...
    async createJobWithEip1193(input: {
    ...
    async createJobWithWalletConnect(input: {
      ...
      return this.createJobWithEip1193(input);
    },
    ...
    async markExpiredWithEip1193(jobId: number, eip1193Provider: unknown) {
    ...
    async markExpiredWithWalletConnect(jobId: number, eip1193Provider: unknown) {
      return this.markExpiredWithEip1193(jobId, eip1193Provider);
    },
    async claimRefundWithEip1193(jobId: number, eip1193Provider: unknown) {
      const browserProvider = new BrowserProvider(eip1193Provider as never);
      const signer = await browserProvider.getSigner();
      const registry = new Contract(config.contracts.registry, registryAbi, signer);
    ...
    async claimRefundWithWalletConnect(jobId: number, eip1193Provider: unknown) {
      return this.claimRefundWithEip1193(jobId, eip1193Provider);
    },
